...
static int HandleResourceChunk(RESOURCEID_CHUNK *resourceid_chunk, uint32_t offset, uint32_t resource_id, int32_t *extra_size) {
    if (offset + 1 <= resourceid_chunk->resourceids_count) {
        ...
        *extra_size += (offset + 1 - resourceid_chunk->resourceids_count) * sizeof(uint32_t);
        ...
        resourceid_chunk->resourceids = (uint32_t *) realloc(resourceid_chunk->resourceids,
...
static int RemoveAttribute(PARSER *manifest_parser, char *tag_name, const char *attr_name, char *attr_value,
    ...
    while (attribute_list) {
        if (strcmp((const char *) sc->strings[attribute_list->name], attr_name) == 0 &&
            strcmp((const char *) sc->strings[attribute_list->string], attr_value) == 0) {
...
static int HandleAttribute(PARSER *ap, OPTIONS *options, int32_t *extra_size) {
    switch (options->mode) {
        case MODE_ADD:
            return AddAttribute(ap, options->tag_name, options->parent_tag, options->deep, options->attr_type,
                                options->attr_name,
                                options->attr_value, options->resource_id, extra_size);
        case MODE_MODIFY:
            return ModifyAttribute(ap, options->tag_name, options->parent_tag, options->deep, options->attr_type,
                                   options->attr_name,
                                   options->attr_value, options->resource_id, extra_size);
        case MODE_REMOVE:
...
static int ModifyTagChunk(PARSER *ap, char *tag_name, char *parent_tag, uint32_t deep, const char *new_tag_name,
                          int32_t *extra_size) {
    XMLCONTENTCHUNK *target_start = NULL;
    uint32_t ori_name;
    STRING_CHUNK *sc = ap->string_chunk;
...
int HandleAXML(PARSER *ap, OPTIONS *options) {
    ...
    if (ap == NULL || ap->string_chunk == NULL || ap->resourceid_chunk == NULL || ap->xmlcontent_chunk == NULL ||
        options == NULL) {
        result = -1;
        goto bail;
    }
